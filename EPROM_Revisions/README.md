...
- total bytes/code bytes/non-code bytes/data bytes/unused bytes are an analysis of the makeup of the EPROM, based on a preliminary disassembly.  total bytes = code bytes + non-code bytes, and non-code bytes = data bytes + unused bytes.
